...
-(void) load:(NSString *)strServerUrl{
  ...
  NSString *strKindEditorPath;
  
  if([strServerUrl isEqualToString:@""])
  {
    //本地HTML编辑器路径
    strKindEditorPath= [[NSBundle mainBundle] pathForResource:@"RichEditor" ofType:@"html" inDirectory:@"KindEditor"];
  }
  else
  {
    strKindEditorPath = strServerUrl;
  }
...
-(void)webViewDidFinishLoad:(UIWebView *)webView
{
  ...
  {
    [jsBridge postNotificationName:@"InitWebHTMLEditorContent" userInfo:[NSDictionary dictionaryWithObjectsAndKeys:content,@"HTMLContent", nil] toWebView:self];
  }
